...
def overall_comparison_by_class(
    ...
    for i in range(n_classes):
        ...
        all_image_in_one_class = torch.zeros(grid_nrow,real.shape[1], real.shape[2], real.shape[3])
        ...
        for model in range(2):
            ...
            for epoch in [10, 100]:
                for embedding in [0, 4, 8, 32]:
                    if embedding == 0:
                        model_path = root_path + f'{data_flag}_OneHot_epo{epoch}_{model_type}.pt'
                    else:
                        model_path = root_path + f'{data_flag}_emb{embedding}_epo{epoch}_{model_type}.pt'
                    # print(model_path)
                    gan.G = torch.load(model_path, map_location=device)
                    samples = gan.generate_img(n_samples, i, n_channels)
                    all_image_in_one_class = torch.cat((all_image_in_one_class, samples), 0)
        axarr[i//2, i%2].imshow(
            torch.permute(make_grid(all_image_in_one_class, nrow=grid_nrow, padding=1), (1,2,0)),
        )
